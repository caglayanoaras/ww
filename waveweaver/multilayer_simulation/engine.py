    ...
    def run(self) -> Dict[str, np.ndarray]:
        ...
        for i in range(len(self.freqs)):
            ...
            for lay in layer_mats:
                ...
                # Note: using -j for forward propagation convention exp(j(wt - kz*z))
                ...
                # Note: X commutes with diagonal matrices, but Ai/Bi are not diagonal.
                # Careful matrix multiplication order required.
                ...
                # D_term = S12_g * (I - S11_l * S22_g)^-1
                # But wait, (I - AB)^-1 * A = A * (I - BA)^-1
                # Let's stick to standard definition:
                # S11 = S11a + S12a * (I - S11b * S22a)^-1 * S11b * S21a
                # Note indices: A=Global, B=Layer
                
                # Redheffer Formula (A=Left, B=Right):
                # Denominator: (I - S11_B * S22_A)^-1
                # But here S_global is built from left to right? 
                # Yes, S_global represents stack so far (Left), S_layer is new layer (Right).
            ...
            # Apply incident polarization to get scalar S-parameters
            # Note: S11, S22 etc are 2x2 matrices relating [TM, TE] modes.
