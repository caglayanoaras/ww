...
import json
import numpy as np
from typing import Dict, Type, Optional, get_origin
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QSplitter,
    QTabWidget, QMenuBar, QApplication,
...
from waveweaver.multilayer_simulation.layer_dialogs import LayerDefinitionDialog
...
class MultilayerSimulationApp(QWidget):
    def __init__(self):
        ...
        self.last_results = None  # Store results for saving
        ...
        self.left_layout.addWidget(self.build_input_panel(), 1)
        ...
        for name in ("Layers", "S-Parameters (dB)", "S-Parameters (Phase)"):
    ...
    def build_input_panel(self) -> QWidget:
        """Builds the Global S-Parameters (TMM) input panel."""
    ...
    def run_calculation(self):
        ...
        try:
            ...
            results = engine.run()
            self.last_results = results  # Store results for saving
    ...
    def save_state_handler(self):
        ...
        try:
            ...
            state = {
                ...
                "results": self._serialize_results(self.last_results)
            }
    ...
    def load_state_handler(self):
        ...
        try:
            ...
            # Accept both the current flat format and the legacy "model_a" format.
            inputs = state.get("inputs")
            results = state.get("results")
            if inputs is None and "model_a" in state:
                model_a = state.get("model_a", {})
                inputs = model_a.get("inputs", {})
                results = model_a.get("results")
            inputs = inputs or {}

            if "fstart" in inputs: self.inp_fstart.setText(inputs["fstart"])
            if "fstop" in inputs: self.inp_fstop.setText(inputs["fstop"])
            if "fpoints" in inputs: self.inp_fpoints.setText(inputs["fpoints"])
            if "theta" in inputs: self.inp_theta.setText(inputs["theta"])
            if "phi" in inputs: self.inp_phi.setText(inputs["phi"])
            if "pte" in inputs: self.inp_pte.setText(inputs["pte"])
            if "ptm" in inputs: self.inp_ptm.setText(inputs["ptm"])

            self.current_layer_data = inputs.get("layers", None)
            if self.current_layer_data:
                self.draw_layers_on_canvas(self.current_layer_data)

            self.last_results = self._deserialize_results(results)
            if self.last_results:
                self.plot_s_parameters(
                    self.last_results['freqs'],
                    self.last_results['S11'],
                    self.last_results['S21'],
                    self.last_results['S12'],
                    self.last_results['S22']
                )
